...
**CRITICAL**: Only submit PRs (run `gt submit`) **after 6 PM local time**.

During working hours the user is actively using the running server and live data. Submitting PRs triggers CI runs and may cause merge queue activity that disrupts the running service.
