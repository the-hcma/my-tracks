...
- **`--refresh`** (first): marker-aware sync (`.github/stacking-tool` → `gh-stack`), prunes merged worktrees and branches, pulls latest main, and ensures the background service is running. Exits immediately — it does **not** prompt for a worktree.
- **plain** (second): repeats the sync/cleanup, then prompts you to name a new worktree for the upcoming work.
- Both commands are required. Prefer `--worktree <stack-name> --no-interactive` when automating.
