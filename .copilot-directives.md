...
### Session Start — Sync main First

**CRITICAL**: At the start of every session, before any other work:
...
**CRITICAL**: Only submit PRs (run `gt submit`) **after 6 PM local time on weekdays (Monday–Friday)**.

PRs may be submitted at any time on weekends (Saturday and Sunday).

During weekday working hours the user is actively using the running server and live data. Submitting PRs triggers CI runs and may cause merge queue activity that disrupts the running service.
...
- ✅ Submit PR: `GRAPHITE_PROFILE=thehcma gt submit --no-interactive --publish`
- ✅ Submit stacked PRs: `GRAPHITE_PROFILE=thehcma gt submit --stack --no-interactive --publish`
...
**CRITICAL**: 
- ❌ **NEVER** run gt commands without `GRAPHITE_PROFILE=thehcma` prefix (authentication will fail)
