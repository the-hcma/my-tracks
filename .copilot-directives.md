...
> See [docs/GRAPHITE.md](./docs/GRAPHITE.md) for the full Graphite workflow reference (branch naming, stack creation, navigation, submission, troubleshooting, and advanced rebasing).
...
- **`--refresh`** (first): syncs main with Graphite (`gt sync`), prunes merged worktrees and branches, pulls latest main, and ensures the background service is running. Exits immediately — it does **not** prompt for a worktree.
- **plain** (second): repeats the sync/cleanup, then prompts you to name a new worktree for the upcoming work.
- Both commands are required. This replaces the manual `gt sync --force` step.
...
- ✅ Create branch + commit: `gt create --all --message "feat: description"`
- ✅ Amend commit: `gt modify --all --message "updated message"`
- ✅ Submit PR: `gt submit --no-interactive --publish`
- ✅ Submit stacked PRs: `gt submit --stack --no-interactive --publish`
- ✅ Sync with remote: `gt sync --force`
- ✅ View stack: `gt log short`
- ✅ Switch branches: `gt checkout <branch>` or `gt up`/`gt down`

**CRITICAL**: 
- ❌ **NEVER** omit `--publish` flag when submitting PRs (creates drafts instead of ready PRs)
