...
from config.runtime import get_actual_mqtt_port, get_mqtt_port, get_mqtt_tls_port
...
class NetworkState:
    ...
    def check_and_update_ips(cls) -> tuple[list[str], bool]:
        ...
        has_changed = cls.last_known_ips is not None and set(cls.last_known_ips) != set(current_ips)
...
def service_worker(request: HttpRequest) -> HttpResponse:
    ...
    path = Path(settings.BASE_DIR) / "web_ui" / "static" / "web_ui" / "sw.js"
    if not path.is_file():
...
def profile(request: HttpRequest) -> HttpResponse:
    ...
    active_cert = (
        ClientCertificate.objects.filter(user=request.user, is_active=True, revoked=False)
        .select_related("issuing_ca")
        .first()
    )
    context["active_cert"] = active_cert
    if active_cert:
        context["cert_meta"] = get_certificate_metadata(active_cert.certificate_pem.encode())

    active_ca = CertificateAuthority.objects.filter(is_active=True).first()
    context["active_ca"] = active_ca
    if active_ca:
        context["ca_meta"] = get_certificate_metadata(active_ca.certificate_pem.encode())
    ...
    context["devices_with_certs"] = [{"device": d, "cert": cert_map.get(d.device_id)} for d in raw_devices]

    # Home location
    context["user_profile"] = user_profile
    last_location = Location.objects.filter(device__owner=request.user).order_by("-timestamp").first()
    context["last_location"] = last_location

    # Geofences tab data
    context["waypoints"] = list(Waypoint.objects.filter(user=request.user, is_active=True).order_by("label"))
    context["geofence_devices"] = list(Device.objects.filter(owner=request.user).order_by("name"))
    context["transitions"] = list(
        Transition.objects.filter(device__owner=request.user)
        .select_related("waypoint", "device")
        .order_by("-timestamp")[:50]
    )
    context["actions"] = list(
        TransitionAction.objects.filter(user=request.user)
...
def geofences(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if form_type == "add_waypoint":
            ...
            wp = get_object_or_404(Waypoint, pk=request.POST["waypoint_id"], user=request.user)
            ...
            device = get_object_or_404(Device, pk=request.POST["device_id"], owner=request.user)
            active_waypoints = list(Waypoint.objects.filter(user=request.user, is_active=True))
            ...
            if email and event in ("enter", "leave", "any"):
                if wp_id_raw == "all":
                    ...
                    user_waypoints = list(Waypoint.objects.filter(user=request.user, is_active=True))
            ...
            action = get_object_or_404(TransitionAction, pk=request.POST["action_id"], user=request.user)
            action.delete()
    ...
    waypoints = list(Waypoint.objects.filter(user=request.user, is_active=True).order_by("label"))
...
def download_my_cert(request: HttpRequest) -> HttpResponse:
    ...
    cert = (
        ClientCertificate.objects.select_related("issuing_ca")
        .filter(user=request.user, is_active=True, revoked=False)
        .first()
    )
    if cert is None:
...
def about(request: HttpRequest) -> HttpResponse:
    ...
    active_sc = (
        ServerCertificate.objects.filter(is_active=True)
        .select_related(
            "issuing_ca",
        )
        .first()
...
def admin_panel(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if form_type == "create_user":
            ...
            if not username:
                ...
                try:
                    ...
                    logger.info(
                        "[http] User '%s' created '%s' (role=%s) via admin panel", request.user.username, username, role
                    )
        ...
        if form_type == "save_location_quality":
            ...
            try:
                ...
                if minimum_m < 1 or minimum_m > 1_000_000:
                    context["location_quality_error"] = "Expected minimum accuracy between 1 and 1000000 meters."
                else:
                    lq = LocationQualitySettings.get_solo()
                    ...
                    logger.info(
                        "[http] Admin '%s' updated location quality filter enabled=%s minimum_accuracy_m=%s",
                        request.user.username,
                        enabled,
                        minimum_m,
                    )
            ...
            if not active_ca_obj:
                ...
                try:
                    ...
                    if validity_days < 1 or validity_days > 36500:
                        ...
                        ca_key_pem = pki_decrypt_private_key(bytes(active_ca_obj.encrypted_private_key))
        ...
        if form_type == "revoke_client_cert":
            ...
            try:
                ...
                if cc.revoked:
                    context["cc_error"] = (
                        f"Certificate for '{cc.common_name}' (fingerprint={cc.fingerprint}) is already revoked."
                    )
                else:
                    cc.revoked = True
                    cc.is_active = False
                    ...
                    cc.save()
                    context["cc_success"] = (
                        f"Certificate for '{cc.common_name}' (fingerprint={cc.fingerprint}) revoked."
                    )
        ...
        if form_type == "expunge_client_cert":
            ...
            try:
                ...
                if cc.is_active and not cc.revoked:
                    ...
                    cc.delete()
                    context["cc_success"] = (
                        f"Certificate for '{cc_name}' (fingerprint={cc_fingerprint}) permanently deleted."
                    )
            except ClientCertificate.DoesNotExist:
                context["cc_error"] = "Client certificate not found."
    ...
    user_id_to_active_cert = {cc.user_id: cc for cc in ClientCertificate.objects.filter(is_active=True, revoked=False)}
    for u in users:
    ...
    context["active_ca"] = active_ca
    if active_ca:
        context["active_ca_meta"] = get_certificate_metadata(active_ca.certificate_pem.encode())
    ...
    if active_sc:
        context["active_sc_meta"] = get_certificate_metadata(active_sc.certificate_pem.encode())
    context["sc_history"] = list(ServerCertificate.objects.all()[:10])

    context["client_certs"] = list(ClientCertificate.objects.select_related("user", "issuing_ca").all()[:50])
    ...
    context["all_waypoints"] = list(Waypoint.objects.select_related("user").order_by("user__username", "label"))

    global_rules = list(
        GlobalAutomationRule.objects.prefetch_related("users").select_related("waypoint", "created_by").order_by("name")
    )
    ...
    for rule in global_rules:
        ...
        if rule.condition == GlobalAutomationRule.CONDITION_ALL_INSIDE:
            ...
            if met:
                ...
                not_inside = [f"{u} is {s}" for u, s in states.items() if s != "inside"]
            ...
            if met:
                ...
                not_outside = [f"{u} is inside" for u, s in states.items() if s == "inside"]
    ...
    raw_transitions = (
        list(
            Transition.objects.filter(waypoint__in=watched_wp_ids)
...
def action_test(request: HttpRequest) -> JsonResponse:
    ...
    action = get_object_or_404(TransitionAction, pk=request.POST.get("action_id"), user=request.user)
...
def smtp_test(request: HttpRequest) -> JsonResponse:
    ...
    active_sc = ServerCertificate.objects.filter(is_active=True).first()
    server_names = get_certificate_sans(active_sc.certificate_pem.encode()) if active_sc else None
