...
class TestProfilePage:
    ...
    def test_profile_page_has_friends_tab(self, logged_in_client: Client) -> None:
        ...
        assert_that(friends_pos, greater_than(devices_pos))
        assert_that(friends_pos, less_than(locations_pos))
...
class TestAdminPanel:
    ...
    def test_admin_panel_shows_user_list(self, admin_logged_in_client: Client, user: User, admin_user: User) -> None:
    ...
    def test_admin_panel_create_duplicate_user(self, admin_logged_in_client: Client, user: User) -> None:
    ...
    def test_admin_panel_shows_delete_button(self, admin_logged_in_client: Client, user: User) -> None:
    ...
    def test_admin_panel_shows_set_password_button(self, admin_logged_in_client: Client, user: User) -> None:
...
class TestAdminPanelPKI:
    ...
    def test_expunge_inactive_ca(self, admin_logged_in_client: Client) -> None:
        ...
        assert_that(CertificateAuthority.objects.filter(pk=old_ca.pk).exists(), is_(False))
    ...
    def test_expunge_active_ca_rejected(self, admin_logged_in_client: Client) -> None:
        ...
        assert_that(CertificateAuthority.objects.filter(pk=active_ca.pk).exists(), is_(True))
...
class TestAdminPanelClientCert:
    ...
    def test_user_dropdown_lists_users(self, admin_logged_in_client: Client) -> None:
        ...
        self._create_ca(admin_logged_in_client)
        self._create_user("dropdownuser")
        response = admin_logged_in_client.get("/admin-panel/")
        content = response.content.decode("utf-8")
...
class TestGeofencesView:
    ...
    def test_get_lists_user_waypoints(self, logged_in_client: Client, user: User) -> None:
        ...
        wp = Waypoint.objects.create(user=user, label="Test Zone", latitude="41.0", longitude="-73.0", radius=200)
        response = logged_in_client.get("/geofences/")
        assert_that(response.status_code, equal_to(status.HTTP_200_OK))
        content = response.content.decode("utf-8")
    ...
    def test_get_excludes_other_users_waypoints(self, logged_in_client: Client, user: User) -> None:
        """Waypoints owned by other users are not shown."""
        from app.models import Waypoint

        other = User.objects.create_user(username="other", password="pass")
        wp = Waypoint.objects.create(user=other, label="Other Zone", latitude="42.0", longitude="-74.0", radius=100)
        response = logged_in_client.get("/geofences/")
        content = response.content.decode("utf-8")
    ...
    def test_post_add_waypoint_creates_object(self, logged_in_client: Client, user: User) -> None:
    ...
    def test_post_edit_waypoint_updates_object(self, logged_in_client: Client, user: User) -> None:
        """POST edit_waypoint updates the waypoint and redirects."""
        from app.models import Waypoint

        wp = Waypoint.objects.create(user=user, label="Old Name", latitude="41.0", longitude="-73.0", radius=100)
        response = logged_in_client.post(
            "/geofences/",
            {
    ...
    def test_post_edit_waypoint_returns_404_for_other_user(self, logged_in_client: Client) -> None:
        """Editing another user's waypoint returns 404."""
        from app.models import Waypoint

        other = User.objects.create_user(username="other2", password="pass")
        wp = Waypoint.objects.create(user=other, label="Theirs", latitude="41.0", longitude="-73.0", radius=100)
        response = logged_in_client.post(
            "/geofences/",
            {
                "form_type": "edit_waypoint",
    ...
    def test_post_delete_waypoint_removes_object(self, logged_in_client: Client, user: User) -> None:
        """POST delete_waypoint removes the waypoint and redirects."""
        from app.models import Waypoint

        wp = Waypoint.objects.create(user=user, label="To Delete", latitude="41.0", longitude="-73.0", radius=100)
        response = logged_in_client.post(
            "/geofences/",
            {
    ...
    def test_post_delete_waypoint_returns_404_for_other_user(self, logged_in_client: Client) -> None:
    ...
    def test_post_sync_to_device_returns_404_for_unowned_device(self, logged_in_client: Client, user: User) -> None:
        """Syncing to a device not owned by the user returns 404."""
        from app.models import Device

        other = User.objects.create_user(username="other4", password="pass")
        device = Device.objects.create(device_id="d1", mqtt_user="other4", name="Device", owner=other)
        response = logged_in_client.post(
            "/geofences/",
            {
    ...
    def test_post_sync_to_device_calls_command_publisher(self, logged_in_client: Client, user: User) -> None:
        """Syncing to an owned device calls CommandPublisher.set_waypoints."""
        from unittest.mock import AsyncMock, patch

        from app.models import Device, Waypoint

        device = Device.objects.create(device_id="mydevice", mqtt_user="testuser", name="My Phone", owner=user)
        Waypoint.objects.create(user=user, label="Home", latitude="41.194", longitude="-73.888", radius=100)
    ...
    def test_waypoint_db_fields_match_set_waypoints_mqtt_json(self, user: User) -> None:
...
class TestProfileTransitions:
    ...
    def test_profile_transitions_in_context(self, logged_in_client: Client, user: User) -> None:
        ...
        device = Device.objects.create(device_id="dev1", mqtt_user="testuser", name="Test Device", owner=user)
        wp = Waypoint.objects.create(user=user, label="Office", latitude="40.7", longitude="-74.0", radius=100)
...
class TestAdminPanelSmtp:
    ...
    def test_smtp_test_includes_public_domain(self, admin_logged_in_client: Client) -> None:
        ...
        def fake_send(
            to: str, backend: SmtpEmailBackend, from_email: str, server_names: list[str] | None = None
        ) -> None:
            captured.append({"to": to, "from_email": from_email})

        with override_settings(PUBLIC_DOMAIN="mytracks.example.com"):
            with patch("web_ui.views.send_test_email", side_effect=fake_send):
    ...
    def test_reset_smtp_button_always_shown(self, admin_logged_in_client: Client) -> None:
        ...
        assert_that(
            html,
            contains_string(
                'form="smtp-reset-form" class="submit-btn" style="background:var(--error,#dc2626)" disabled'
            ),
        )
