...
class TestDockerCompose:
    ...
    def test_compose_file_exists(self) -> None:
        assert_that((_ROOT / "production/docker/docker-compose.yml").exists(), is_(True))

    def test_defines_my_tracks_service(self) -> None:
        assert_that(_read("production/docker/docker-compose.yml"), contains_string("my-tracks:"))

    def test_defines_nginx_service(self) -> None:
        assert_that(_read("production/docker/docker-compose.yml"), contains_string("nginx:"))

    def test_defines_certbot_service(self) -> None:
        assert_that(_read("production/docker/docker-compose.yml"), contains_string("certbot:"))

    def test_certbot_is_optional_profile(self) -> None:
        source = _read("production/docker/docker-compose.yml")
    ...
    def test_no_database_url_in_base(self) -> None:
        assert_that(
            _read("production/docker/docker-compose.yml"), is_not(contains_string("DATABASE_URL"))
        )

    def test_nginx_exposes_443(self) -> None:
        assert_that(_read("production/docker/docker-compose.yml"), contains_string("443"))

    def test_nginx_exposes_8883(self) -> None:
        assert_that(_read("production/docker/docker-compose.yml"), contains_string("8883"))

    def test_certs_volume_mounted(self) -> None:
        assert_that(_read("production/docker/docker-compose.yml"), contains_string("/etc/nginx/certs"))

    def test_env_production_referenced(self) -> None:
        assert_that(_read("production/docker/docker-compose.yml"), contains_string(".env.production"))
...
class TestDockerComposePostgresTesting:
    ...
    def test_postgres_compose_file_exists(self) -> None:
        assert_that((_ROOT / "production/docker/docker-compose.postgres-testing.yml").exists(), is_(True))

    def test_defines_postgres_service(self) -> None:
        assert_that(
            _read("production/docker/docker-compose.postgres-testing.yml"), contains_string("postgres:")
        )

    def test_postgres_has_healthcheck(self) -> None:
        assert_that(
            _read("production/docker/docker-compose.postgres-testing.yml"), contains_string("pg_isready")
        )

    def test_my_tracks_depends_on_postgres(self) -> None:
        assert_that(
            _read("production/docker/docker-compose.postgres-testing.yml"), contains_string("service_healthy")
        )

    def test_postgres_data_volume_defined(self) -> None:
        assert_that(
            _read("production/docker/docker-compose.postgres-testing.yml"), contains_string("postgres-data:")
        )
...
class TestDockerComposePostgresProduction:
    ...
    def test_postgres_compose_file_exists(self) -> None:
        assert_that((_ROOT / "production/docker/docker-compose.postgres-production.yml").exists(), is_(True))

    def test_defines_postgres_service(self) -> None:
        assert_that(
            _read("production/docker/docker-compose.postgres-production.yml"), contains_string("postgres:")
        )

    def test_postgres_has_healthcheck(self) -> None:
        assert_that(
            _read("production/docker/docker-compose.postgres-production.yml"), contains_string("pg_isready")
        )

    def test_my_tracks_depends_on_postgres(self) -> None:
        assert_that(
            _read("production/docker/docker-compose.postgres-production.yml"), contains_string("service_healthy")
        )

    def test_postgres_data_uses_bind_mount(self) -> None:
        assert_that(
            _read("production/docker/docker-compose.postgres-production.yml"),
...
class TestNginxConfig:
    ...
    def test_nginx_conf_exists(self) -> None:
        assert_that((_ROOT / "production/nginx/nginx.conf").exists(), is_(True))

    def test_proxy_params_exists(self) -> None:
        assert_that((_ROOT / "production/nginx/proxy_params").exists(), is_(True))

    def test_https_server_block(self) -> None:
        source = _read("production/nginx/nginx.conf")
    ...
    def test_forwarded_proto_header(self) -> None:
        source = _read("production/nginx/proxy_params")
...
class TestEnvProductionExample:
    ...
    def test_env_production_example_exists(self) -> None:
        assert_that((_ROOT / "examples/.env.production.example").exists(), is_(True))

    def test_contains_secret_key(self) -> None:
        assert_that(_read("examples/.env.production.example"), contains_string("SECRET_KEY="))

    def test_contains_debug_false(self) -> None:
        assert_that(_read("examples/.env.production.example"), contains_string("DEBUG=False"))

    def test_contains_database_url(self) -> None:
        assert_that(_read("examples/.env.production.example"), contains_string("DATABASE_URL="))

    def test_contains_postgres_password(self) -> None:
        assert_that(_read("examples/.env.production.example"), contains_string("POSTGRES_PASSWORD="))

    def test_contains_allowed_hosts(self) -> None:
        assert_that(_read("examples/.env.production.example"), contains_string("ALLOWED_HOSTS="))

    def test_contains_csrf_trusted_origins(self) -> None:
        assert_that(_read("examples/.env.production.example"), contains_string("CSRF_TRUSTED_ORIGINS="))

    def test_contains_certs_dir(self) -> None:
        assert_that(_read("examples/.env.production.example"), contains_string("CERTS_DIR"))
