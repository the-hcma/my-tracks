...
class RequestLoggingMiddleware:
    ...
    def __call__(self, request: HttpRequest) -> HttpResponse:
        ...
        if req_body is not None and resp_body is not None:
            logger.log(level, "%s %s -> %s | req: %s | resp: %s",
                       request.method, request.path, response.status_code,
                       req_body, resp_body)
        elif req_body is not None:
            logger.log(level, "%s %s -> %s | req: %s",
                       request.method, request.path, response.status_code, req_body)
        elif resp_body is not None:
            logger.log(level, "%s %s -> %s | resp: %s",
                       request.method, request.path, response.status_code, resp_body)
        else:
