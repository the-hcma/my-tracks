...
from .models import (CertificateAuthority, ClientCertificate, Device,
                     DeviceShare, FriendRequest, Location, OwnTracksMessage,
                     ServerCertificate)
...
from .serializers import (CertificateAuthoritySerializer,
                          ...
                          DeviceShareSerializer, FriendRequestSerializer,
                          FriendSerializer, LocationSerializer,
                          ServerCertificateSerializer, UserProfileSerializer,
                          UserSerializer)
...
class FriendRequestViewSet(viewsets.ViewSet):
    ...
    def create(self, request: Request) -> Response:
        ...
        FriendRequest.objects.filter(
            from_user=request.user, to_user=target, status=FriendRequest.DECLINED
        ).delete()

        # Already friends?
        already_friends = FriendRequest.objects.filter(
            from_user=request.user, to_user=target, status=FriendRequest.ACCEPTED
        ).exists() or FriendRequest.objects.filter(
            from_user=target, to_user=request.user, status=FriendRequest.ACCEPTED
        ).exists()
        if already_friends:
            return Response(
                {"error": "Already friends"}, status=status.HTTP_409_CONFLICT
            )
        ...
        return Response(
            FriendRequestSerializer(req).data, status=status.HTTP_201_CREATED
        )
    ...
    def destroy(self, request: Request, pk: str | None = None) -> Response:
        ...
        try:
            req = FriendRequest.objects.get(
                pk=pk, from_user=request.user, status=FriendRequest.PENDING
            )
        except FriendRequest.DoesNotExist:
            return Response({"error": "Not found"}, status=status.HTTP_404_NOT_FOUND)
        req.delete()
...
class FriendViewSet(viewsets.ViewSet):
    ...
    def destroy(self, request: Request, pk: str | None = None) -> Response:
        ...
        DeviceShare.objects.filter(
            device__owner=request.user, shared_with=other
        ).delete()
        DeviceShare.objects.filter(
            device__owner=other, shared_with=request.user
        ).delete()
...
class DeviceShareViewSet(viewsets.ViewSet):
    ...
    def list(self, request: Request, user_id: str = "") -> Response:
        ...
        if err:
            return err
        qs = DeviceShare.objects.filter(
            device__owner=request.user, shared_with=friend
        ).select_related("device", "shared_with")
    ...
    def create(self, request: Request, user_id: str = "") -> Response:
        ...
        share, created = DeviceShare.objects.get_or_create(
            device=device, shared_with=friend
        )
        return Response(
    ...
    def destroy(
        self, request: Request, user_id: str = "", device_id: str = ""
    ) -> Response:
